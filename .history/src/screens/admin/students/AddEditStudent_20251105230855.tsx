...
export const AddEditStudent: React.FC = () => {
  ...
  const onSubmit = async (values: any, formikHelpers: any) => {
    ...
    try {
      ...
      // Safely navigate back to StudentList if it's present in the current navigator state,
      // otherwise fall back to goBack() so we don't trigger "not handled by navigator" errors.
      const state: any = navigation.getState?.();
      const hasStudentList = !!(
        state && Array.isArray(state.routes) && state.routes.some((r: any) => r.name === 'StudentList')
      );
      if (hasStudentList) {
        ...
        // As a last resort, try navigate but wrapped to avoid runtime errors
        try {
          navigation.navigate('StudentList' as never);
        } catch (e) {
          // no-op
    ...
    <ReusableForm
      ...
      cancelAction={() => {
        ...
        if (hasStudentList) {
          ...
          try { navigation.navigate('StudentList' as never); } catch (e) {}
        }
      }}
      tNamespace="student"
    />
