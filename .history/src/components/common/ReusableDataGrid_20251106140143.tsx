...
export const ReusableDataGrid: React.FC<ReusableDataGridProps> = ({
  ...
  const [gridFilters] = useState(filters);
    ...
    if (!fetchUrl) {
      const filteredData = clientSideData.filter((item) => {
        ...
        if ((latestFilters.current as any).schoolId) {
          isMatch = isMatch && item.schoolId === (latestFilters.current as any).schoolId;
        }
        if ((latestFilters.current as any).classId) {
          isMatch = isMatch && item.classId === (latestFilters.current as any).classId;
        }
        if ((latestFilters.current as any).divisionId) {
          isMatch = isMatch && item.divisionId === (latestFilters.current as any).divisionId;
        }
        if (searchText) {
  ...
  useEffect(() => {
    // fetchData is wrapped with useCallback and included as a dependency
    // to satisfy the Hooks lint rule and ensure latest dependencies are used.
    fetchData();
  }, []);
  ...
  const handleEdit = (item: any) => editUrl && (navigation as any).navigate(editUrl, { id: item.id });
  ...
  const renderItem = ({ item }: { item: any }) => {
    const idKey = item.id || item._id || String(item[columns[0]?.key] || Math.random());
    ...
    const openMenu = (key: string) => setMenuVisibleFor((p) => ({ ...p, [key]: true }));
    const closeMenu = (key: string) => setMenuVisibleFor((p) => ({ ...p, [key]: false }));
      ...
      <Card style={[styles.card, { backgroundColor: '#ffffff' }]} elevation={4}>
        <Card.Content>
          <View style={styles.cardHeaderRow}>
            <View style={{ flex: 1 }}>
              <Text variant="labelLarge" style={{ color: '#111111', fontWeight: '700' }}>{primaryTitle}</Text>
              {/* small meta row */}
              <View style={styles.metaRow}>
                <Chip mode="outlined" style={[styles.idChip, { backgroundColor: '#f3f3f3' }]}>{item.id || item._id || ''}</Chip>
                <Text style={{ marginLeft: 8, color: '#af0303ff' }}>{item.visibility || ''}</Text>
              </View>
            </View>
            ...
            <View style={styles.headerActions}>
              <Menu
                ...
                anchor={<IconButton icon="dots-vertical" onPress={() => openMenu(idKey)} />}
              >
                <Menu.Item onPress={() => { closeMenu(idKey); handleEdit(item); }} title="Edit" />
                <Menu.Item onPress={() => { closeMenu(idKey); /* manage visibility */ }} title="View" />
                {deleteUrl && <Menu.Item onPress={() => { closeMenu(idKey); showDeleteDialog(item.id || item._id); }} title="Delete" />}
              </Menu>
            </View>
          </View>
          ...
          {columns.slice(1, 4).map((col) => (
            <View key={col.key} style={styles.dataRow}>
              <Text variant="labelSmall" style={{ color: '#000000ff' }}>{col.header}</Text>
              <Text variant="bodyMedium" style={{ color: '#111111' }}>{item[col.key] ?? '—'}</Text>
            </View>
          ))}

          {isExpanded && columns.slice(4).map((col) => (
            <View key={col.key} style={styles.dataRow}>
              <Text variant="labelSmall" style={{ color: '#ffffffff' }}>{col.header}</Text>
              <Text variant="bodyMedium" style={{ color: '#111111' }}>{item[col.key] ?? '—'}</Text>
            </View>
          ))}

          <View style={styles.viewMoreRow}>
            <Button mode="text" onPress={() => setExpandedId(isExpanded ? null : idKey)}>
  ...
  return (
    ...
    <View style={[styles.headerContainer, { backgroundColor: '#ffffff', borderBottomColor: '#e6e6e6' }]}>
      <Text variant="headlineMedium" style={{ color: '#111111' }}>{title}</Text>
            <IconButton 
              icon="reload" 
              size={24} 
              onPress={onRefresh} 
      ...
      {loading && !refreshing ? <LoadingSpinner /> : (
        <FlatList
          ...
          ListFooterComponent={showPagination ? () => (
            <View style={[styles.pagination, { borderTopColor: theme.colors.outlineVariant }]}>
              <Button 
      ...
      <Portal>
        <Dialog visible={isDeleteDialogVisible} onDismiss={hideDeleteDialog}>
          ...
          <Dialog.Actions>
            <Button onPress={hideDeleteDialog} mode="outlined">Cancel</Button>
            <Button onPress={handleDelete} mode="contained" buttonColor={theme.colors.error}>Delete</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </SafeAreaView>
