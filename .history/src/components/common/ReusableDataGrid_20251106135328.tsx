...
export const ReusableDataGrid: React.FC<ReusableDataGridProps> = ({
      ...
      <Card style={[styles.card, { backgroundColor: theme.colors.surface }]} elevation={4}>
        <Card.Content>
          <View style={styles.cardHeaderRow}>
            <View style={{ flex: 1 }}>
              <Text variant="labelLarge" style={{ color: theme.colors.primary, fontWeight: '700' }}>{primaryTitle}</Text>
              {/* small meta row */}
              <View style={styles.metaRow}>
                <Chip mode="outlined" style={styles.idChip}>{item.id || item._id || ''}</Chip>
                <Text style={{ marginLeft: 8, color: theme.colors.onSurfaceVariant }}>{item.visibility || ''}</Text>
              </View>
            </View>
            ...
            <View style={styles.headerActions}>
              ...
              >
                <Menu.Item onPress={() => { closeMenu(idKey); handleEdit(item); }} title="View & Edit" />
                <Menu.Item onPress={() => { closeMenu(idKey); /* manage visibility */ }} title="Manage Visibility" />
          ...
          {columns.slice(1, 4).map((col) => (
            <View key={col.key} style={styles.dataRow}>
              <Text variant="labelSmall" style={{ color: theme.colors.onSurfaceVariant }}>{col.header}</Text>
              <Text variant="bodyMedium" style={{ color: theme.colors.onSurface }}>{item[col.key] ?? '—'}</Text>
            </View>
          ))}
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        <View style={[styles.headerContainer, { backgroundColor: theme.colors.elevation.level2, borderBottomColor: theme.colors.outlineVariant }]}>
            <Text variant="headlineMedium" style={{ color: theme.colors.onSurface }}>{title}</Text>
            <IconButton 
              icon="reload" 
              size={24} 
              onPress={onRefresh} 
