...
export const ReusableForm: React.FC<ReusableFormProps> = ({
  ...
  const nounLabel = entityName || (tNamespace ? tNamespace.charAt(0).toUpperCase() + tNamespace.slice(1) : undefined) || 'Item';
    ...
    if (formikHelpers) {
      return (
        <TextInput
          ...
          error={!!(formikHelpers.touched[field.name] && formikHelpers.errors[field.name])}
          style={[styles.input]}
                ...
                {(formikProps) => (
                  <>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                      {showCancelButton && (
                        <Button mode="outlined" onPress={() => (cancelAction ? cancelAction() : navigation.goBack())} style={[styles.cancelButton, { borderColor: theme.colors.outlineVariant || 'lightgray' }]} disabled={false}>
                          Cancel
                        </Button>
                      )}
                      <Button mode="contained" onPress={() => formikProps.handleSubmit()} style={styles.saveButton} loading={formikProps.isSubmitting} disabled={formikProps.isSubmitting} buttonColor={theme.colors.primary} textColor={theme.colors.onPrimary || 'white'}>
