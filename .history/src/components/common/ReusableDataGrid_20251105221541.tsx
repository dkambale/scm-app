...
export const ReusableDataGrid: React.FC<ReusableDataGridProps> = ({
  ...
  const renderItem = ({ item }: { item: any }) => {
    // Determine the primary field to use as the card title (e.g., the first column)
    ...
    return (
      <Card 
        ...
        elevation={4}
        onPress={() => editUrl && handleEdit(item)} // Make the whole card tappable for edit
      >
        <Card.Title 
          title={primaryTitle}
          subtitle={entityName}
          ...
          left={(props) => <IconButton {...props} icon="information-outline" />}
        />
        <Card.Content style={styles.cardContent}>
          {columns.slice(1).map((col, index) => ( // Render remaining columns
            <View key={col.key} style={styles.dataRow}>
                <Text variant="labelMedium" style={{ color: theme.colors.onSurfaceVariant }}>{col.header}</Text>
                <View style={styles.dataValueContainer}>
                  {col.renderCell ? col.renderCell(item) : <Text variant="bodyMedium" style={{ color: theme.colors.onSurface }}>{item[col.key]}</Text>}
                </View>
            </View>
          ))}
        </Card.Content>
        ...
        <Card.Actions style={styles.actions}>
            {/* Action buttons are now concise and high-contrast */}
            {editUrl && (
              <Button 
                icon="pencil" 
                mode="text" 
