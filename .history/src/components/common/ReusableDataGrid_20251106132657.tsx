...
export const ReusableDataGrid: React.FC<ReusableDataGridProps> = ({
  ...
  // --- LIGHT THEME UI OVERHAUL (White BG, Black Text, Blue Accent) ---
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <Card 
        ...
        style={[styles.card, { backgroundColor: theme.colors.surface || 'white' }]} 
        elevation={4}
        ...
        <Card.Content style={styles.cardContent}>
          {columns.slice(secondaryColumn ? 2 : 1).map((col) => (
            <View key={col.key} style={styles.dataRow}>
                ...
                <Text variant="labelMedium" style={{ color: theme.colors.onSurfaceVariant || 'black' }}>{col.header}</Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background || 'white' }]}>
        
        {/* Sticky Header with White BG and Black/Blue Text */}
        <View style={[styles.headerContainer, { 
          backgroundColor: theme.colors.elevation.level1 || 'white', 
          borderBottomColor: theme.colors.outlineVariant || '#ddd',
          ...Platform.select({ ios: { zIndex: 1 } }) // Ensure header is above list on iOS
        }]}>
            {/* Black text for title */}
            <Text variant="headlineMedium" style={{ color: theme.colors.onSurface || 'black' }}>{title}</Text>
            {/* Blue icon for refresh */}
            <IconButton 
              icon="reload" 
              size={24} 
              onPress={onRefresh} 
        ...
        {/* Search Bar with White BG, Black Text, Blue Accent */}
        <View style={styles.searchContainer}>
          <Searchbar
            ...
            // Explicitly set colors for light theme contrast
            placeholderTextColor={theme.colors.onSurfaceVariant || 'gray'}
            iconColor={theme.colors.onSurfaceVariant || 'gray'}
            inputStyle={{ 
                color: theme.colors.onSurface || 'black', // Input text color
                minHeight: 0 
            }}
            style={[styles.searchbar, { backgroundColor: theme.colors.surface || 'white' }]}
          />
        </View>
      ...
      {loading && !refreshing ? <LoadingSpinner /> : (
        <FlatList
          ...
          ListFooterComponent={showPagination ? () => (
            <View style={[styles.pagination, { borderTopColor: theme.colors.outlineVariant || '#ddd' }]}>
              <Button 
      ...
      {/* FAB (Floating Action Button) for Add Action - Blue BG, White Icon */}
      {addActionUrl && (
        <FAB
      ...
      <Portal>
        <Dialog 
            ...
            // Dialog surface is white
            style={{ backgroundColor: theme.colors.surface || 'white' }}
        >
          {/* Black text for dialog content */}
...
const styles = StyleSheet.create({
  ...
  emptyContainer: { 
    ...
    borderRadius: 12,
    borderWidth: 1,
    borderColor: '#ddd', // Light border for light theme
  },
  pagination: { 
    flexDirection: 'row', 
    justifyContent: 'space-between',
