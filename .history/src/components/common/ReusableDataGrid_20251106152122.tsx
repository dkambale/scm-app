...
export const ReusableDataGrid: React.FC<ReusableDataGridProps> = ({
  ...
  useEffect(() => {
    // Run fetchData on mount and whenever key inputs change.
    ...
    fetchData();
  }, [fetchData, gridFilters, paginationModel.page, paginationModel.pageSize]);

  const handleFiltersChange = (newFilters: any) => {
    setGridFilters(newFilters);
    latestFilters.current = newFilters;
    // reset to first page; the effect watching paginationModel/gridFilters will trigger fetch
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: "#ffffff" }]}>
      ...
      {Platform.OS === "android" && actionsModalId && (
        <Modal
          ...
          animationType="fade"
          transparent={true}
          onRequestClose={() => setActionsModalId(null)}
        >
          <View style={styles.modalOverlay}>
            <View style={styles.actionsModalContent}>
              {/* find the item by id */}
              {(() => {
                const sel = gridData.find((it) => {
                  const keyVal = it.id ?? it._id ?? String(it[columns[0]?.key] ?? "");
                  return String(keyVal) === String(actionsModalId);
                });
                return (
                  <>
                    <Button
                    ...
                    {deleteUrl && (
                      <Button
                        ...
                        onPress={() => {
                          setActionsModalId(null);
                          if (sel) showDeleteDialog(sel.id || sel._id);
                        }}
                      >
                        Delete
                      </Button>
                    )}
                    <Button onPress={() => setActionsModalId(null)}>Close</Button>
                  </>
                );
              })()}
            </View>
